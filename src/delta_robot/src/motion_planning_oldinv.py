...
class motion_planning:
	def __init__(self, xy_limits, z_start=0.15):
		...
		self.speed_pub = rospy.Publisher("/asparagus_cluster", Int32, queue_size=1)
		...
		self.place_poz = [[-0.09, 0.340, 0.20, 0, self.gripper_close_poz], [-0.09, -0.340, 0.20, 0, self.gripper_close_poz]]
		self.intermediate_point1 = [[-0.09, 0.20, 0.20, 0, self.gripper_close_poz], [-0.09, -0.20, 0.20, 0, self.gripper_close_poz]]
		self.intermediate_point2 = [[-0.09, 0.30, 0.20, 0, self.gripper_close_poz], [-0.09, -0.30, 0.20, 0, self.gripper_close_poz]]
		...
		# maxJointSpeed = [210,210,210,250,250]
		...
		self.cas_zacetka = time.time() # zacetek pobiranja
		self.stevilo = 1 # stevilo pobranih
	...
	def callback_track_pose(self, data):
		...
		# print("inv 0")
		# qd_radians, eror = deltaInvKin(self.next_point[0], self.next_point[1], self.next_point[2])
		# if any(eror):
		# 		print("point 1", self.next_point[:3])
		# 		rospy.sleep(3)
		...
		if status > 0 and self.pub_idx == 1:
			...
			# print("inv 1")
			# qd_radians, eror= deltaInvKin(r_control[0], r_control[1], r_control[2])
			# if any(eror):
			# 	print("point 2", r_control[:3])
			# 	rospy.sleep(3)
	...
	def callback_robot_angles(self, data):
		...
		if self.first_flag:
			...
			# qd_radians, eror = deltaInvKin(self.next_point[0], self.next_point[1], self.next_point[2])
			# if any(eror):
			# 	print("point 3", self.next_point[:3])
			# 	rospy.sleep(3)
			desired_angle_deg = 180 / np.pi *(qd_radians)
	...
	def callback_aspragus_locations(self, data):
		...
		# print("lokacije", self.asparagus_locations)
		...
		# print("got location: ", self.asparagus_locations)



		# Plan path for robot	
		if self.new_path_request:
			# print("new request za path", self.new_path_request)
			# print("lokacije", self.asparagus_locations)
	...
	def locations_out_of_range(self, asparagus_locations):
		...
		for idx, i in enumerate(asparagus_locations):
			...
			if y_robot_cs < self.y_min_limit or y_robot_cs > self.y_max_limit:
				delete_idx = np.append(delete_idx, idx)
		# print("unclean", self.asparagus_locations_robot_cs)
		# Delete points that are out of range
		delete_idx = np.array(delete_idx, dtype=int)
		if len(delete_idx) > 0:
			self.cleaned_locations = np.delete(self.asparagus_locations, delete_idx, axis=0)
			self.cleaned_locations_robot_cs = np.delete(self.asparagus_locations_robot_cs, delete_idx, axis=0)
			# print("skipped: ", delete_idx)
		...
		if self.step_path != 0:
			if len(asparagus_locations) > 1:
				i = self.pick_idx
				xy_coordinates = asparagus_locations[:, :2]
				distances = np.linalg.norm(xy_coordinates[:, np.newaxis] - xy_coordinates, axis=-1)
				distances = distances[i]
				dist_cut = distances[i:]
				count = np.count_nonzero((dist_cut < 0.2) & (dist_cut > 0))
				...
				# 	self.wait = True
				# 	self.old_idx =i
				# if (i-self.old_idx) < 2 and (i-self.old_idx) != 0:
				# 	if self.slow:
				# 		count = 1
				# 	if self.wait:
				# 		count = 2
				...
				# print("index: ", i)
				# print("distances", dist_cut)
				# print("count", count)
				# print("--------")
				self.speed_pub.publish(count)

		
	
		# print("pocisceno robo", self.cleaned_locations_robot_cs)
	...
	def path_planning(self, min_dist, pretarget_dist, calculation_step):
		...
		if self.step_path == 0:
			...
			ena = np.array(self.cleaned_locations)
			dva = ena.reshape(-1, 4)
			# print("picked asparaus",self.picked_asparagus )
			# print("cleaned locations: ", dva)
			# print("prej: ",self.picked_asparagus_flag_arr)
			# print("")
			if len(self.picked_asparagus) > 0:
				...
				for idx, i in enumerate(self.picked_asparagus):
					# print("prvi for idx: ", idx, "i", i)
					for idx2, j in enumerate(dva):
						# print("drugi for idx2: ", idx2, "j", j)
						dist = np.linalg.norm(j[:3] - i[:3])
						# print("dist", dist)
						# print("***********************************************")
						self.pick_dist_array[idx, idx2] = dist
				# print("pick_dist_array: ", self.pick_dist_array)
				# print("")
				...
				for i in self.pick_dist_array:
					...
					# print("min distance measured", min_dist_measured)
					if min_dist_measured < 0.02:
						min_idx =np.argmin(i)
						# print("argmin od i",min_idx )
						# print("")
						self.picked_asparagus_flag_arr[min_idx] = True
						# print("po: ",self.picked_asparagus_flag_arr)
						# print("")
			...
			if len(self.cleaned_locations) != len(self.asparagus_too_close):
				...
				# print("-----------------")
				# print("popo", self.picked_asparagus_flag_arr)
				# print("klin ", self.cleaned_locations)
				# print("~~~~~~~~~~~~")
				...
				# 		print("new new temp: ", temp)
					
				ena = np.array(self.cleaned_locations)
				dva = ena.reshape(-1, 4)	
				flag_array = np.zeros(len(dva), dtype=bool)
				threshold = 0.025
				for i, det_point in enumerate(dva):
					flag_array[i] = False  
					for pick_point in self.picked_asparagus:
				...
				self.picked_asparagus_flag_arr = flag_array
				print("----------------------")
	...
	def robot_motion(self):
		...
		if self.step_motion == 0:
			...
			# qd_radians[:3], eror = deltaInvKin(temp[0], temp[1], temp[2])
			# if any(eror):
			# 	print("point 4", temp[:3])
			# 	rospy.sleep(3)
			...
			if self.path_calculated:
				...
				rospy.loginfo("POBERI " + str(self.stevilo))
			
				self.cas_zacetka = time.time()
			...
			# qd_radians[:3], eror = deltaInvKin(temp[0], temp[1], temp[2])
			# if any(eror):
			# 	print("point 5", temp[:3])
			# 	rospy.sleep(3)

			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])

			# angle_error = [3, 3, 3, 1, 1] #original
			angle_error = [5, 5, 5, 2, 2]
			self.robot_in_poz = self.robot_in_position(qd_radians, self.qq, angle_error)
			# print("2")
			if self.robot_in_poz:
				# print(str(self.robot_in_poz) + " " + str(self.step_motion))
			...
			# qd_radians[:3], eror= deltaInvKin(temp[0], temp[1], temp[2])
			# if any(eror):
			# 	print("point 7", temp[:3])
			# 	rospy.sleep(3)

			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])

			# angle_error = [3, 3, 3, 1, 1]
			angle_error = [10, 10, 10, 10, 10]
			self.robot_in_poz = self.robot_in_position(qd_radians, self.qq, angle_error)
			# print("5")
			if self.robot_in_poz:
				# print(str(self.robot_in_poz) + " " + str(self.step_motion))
				...
				print("Picked spargel: ", self.picked_asparagus)
				self.first_asparagus_picked = True

				self.path_calculated = False
				self.new_path_request = True

				self.step_motion = 6.5
				self.goNext = True

		elif self.step_motion == 6.5:
			if self.goNext:
				print("Motion", self.step_motion)
				self.goNext = False
			# Intermediate point toward drop off
			...
			self.set_poz = self.intermediate_point1[self.storage_idx]
			qd_radians = np.zeros(5)
			temp = self.next_point
			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])

			angle_error = [5, 5, 5, 5, 5]
			self.robot_in_poz = self.robot_in_position(qd_radians, self.qq, angle_error)
			...
			if self.robot_in_poz:
				...
				self.step_motion = 6.75
				self.goNext = True

		elif self.step_motion == 6.75:
			if self.goNext:
				print("Motion", self.step_motion)
				self.goNext = False
			# Intermediate point toward drop off
			...
			self.set_poz = self.intermediate_point2[self.storage_idx]
			qd_radians = np.zeros(5)
			temp = self.next_point
			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])
			...
			if elapsed_time > self.gripper_time:
				...
				rospy.loginfo("ODLOZI "+ str(self.stevilo))
				self.odlozi = True
				casPobiranja = time.time()-self.cas_zacetka
				# rospy.loginfo("Cas pobiranja: " + str(casPobiranja))
				self.stevilo += 1

				self.step_motion = 8.5
				self.goNext = True

		elif self.step_motion == 8.5:
			if self.goNext:
				print("Motion", self.step_motion)
				self.goNext = False
			# Intermediate point after toward drop off
			# Check y coordinate of the gripper -> go to closer storage
			if self.set_poz[1] >= 0:
				# Go to left storage
				self.storage_idx = 0
			...
			if self.robot_in_poz:
				...
				self.step_motion = 8.75
				self.goNext = True
		
		elif self.step_motion == 8.75:
			if self.goNext:
				print("Motion", self.step_motion)
				self.goNext = False
			# Intermediate point after toward drop off
