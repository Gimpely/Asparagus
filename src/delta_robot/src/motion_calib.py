...
class motion_planning:
	def __init__(self, xy_limits, z_start=0.15):
		...
		rospy.init_node("motion_fixed_points")
	...
	def convert_robot_cs_to_point(self, point, z_offset):
		point_robot = np.zeros(len(point))
	...
	def robot_motion(self):
		...
		if self.step_motion == 0:
			...
			if self.robot_in_poz:
				...
				print("I. tocka home  opzicija", self.set_poz)
		...
		###############################
			...
			self.path_idx = 0
			# go to next poz
			next_poz = [200, 0, -700, 0, self.gripper_open_poz]
			self.set_poz = self.convert_robot_cs_to_point(next_poz, self.z_offset)
			self.step_motion = 3
			...
			if self.robot_in_poz:
				...
				print("II. tocka pobiranja  opzicija", self.set_poz)
		...
		###############################
			...
			self.path_idx = 0
			# go to next poz
			next_poz = [211, -6, -875, 0, self.gripper_close_poz]
			self.set_poz = self.convert_robot_cs_to_point(next_poz, self.z_offset)
			self.step_motion = 5
		...
		###############################
			...
			if self.robot_in_poz:
				...
				print("IV. tocka levo  opzicija", self.set_poz)
		...
		### Tocka nad pobiranjem ###
		###############################
		elif self.step_motion == 8:
			self.robot_in_poz = False
			self.path_idx = 0
			# go to next poz
			next_poz = [209, -6, -800, 0, self.gripper_close_poz]
			self.set_poz = self.convert_robot_cs_to_point(next_poz, self.z_offset)
			self.step_motion = 9
			...
			if self.robot_in_poz:
				...
				print("V. tocka nad pobiranjem  opzicija", self.set_poz)
		...
		### Tocka pobiranja ###
		###############################
		elif self.step_motion == 10:
			self.robot_in_poz = False
			self.path_idx = 0
			# go to next poz
			next_poz = [209, -6, -875, 0, self.gripper_close_poz]
			self.set_poz = self.convert_robot_cs_to_point(next_poz, self.z_offset)
			self.step_motion = 11

		elif self.step_motion == 11:
			# Check if we reached home position
			qd_radians = np.zeros(5)
			temp = self.convert_point_to_robot_cs(self.set_poz, self.z_offset)
			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])
			...
			if self.robot_in_poz:
				...
				print("VI. tocka pobiranja  opzicija", self.set_poz)
				self.CloseGripper = False
				if  self.qq[4] < - 97:
					self.robot_in_poz = False
					self.robot_in_poz_inter = True
		...
		### Tocka odlaganja desno ###
		###############################
		elif self.step_motion == 12:
			self.robot_in_poz = False
			self.path_idx = 0
			# go to next poz
			next_poz = [-249, 0, -879, 0, self.gripper_close_poz]
			self.set_poz = self.place_poz[1]
			self.step_motion = 13

		elif self.step_motion == 13:
			# Check if we reached home position
			qd_radians = np.zeros(5)
			temp = self.convert_point_to_robot_cs(self.set_poz, self.z_offset)
			qd_radians[:3], _ = deltaInverseKin(temp[0], temp[1], temp[2], temp[3])
			...
			if self.robot_in_poz:
				...
				print("IV. tocka desno  opzicija", self.set_poz)
